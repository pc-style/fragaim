...
class Utils:
    ...
    def check_key_binds(self):  # Return a boolean based on if the config needs to be reloaded
        if self.key_reload_config and win32api.GetAsyncKeyState(self.key_reload_config) < 0:
            return True

        if self.key_toggle_aim and win32api.GetAsyncKeyState(self.key_toggle_aim) < 0:
            self.aim_state = not self.aim_state
            print("AIM: " + str(self.aim_state))
            sleep(self.delay)

        if self.key_toggle_recoil and win32api.GetAsyncKeyState(self.key_toggle_recoil) < 0:
            self.recoil_state = not self.recoil_state
            print("RECOIL: " + str(self.recoil_state))
            sleep(self.delay)
        
        if self.key_exit and win32api.GetAsyncKeyState(self.key_exit) < 0:
            print("Exiting")
            exit(1)
        return False
        ...
        if self.aim_state:
            if self.aim_keys[0] == 'off':
                ...
                for key in self.aim_keys:
                    if isinstance(key, int) and win32api.GetAsyncKeyState(key) < 0:
                        return True
        return False

    def get_trigger_state(self):
        if self.key_trigger and win32api.GetAsyncKeyState(self.key_trigger) < 0:
            return True
        return False

    def get_rapid_fire_state(self):
        if self.key_rapid_fire and win32api.GetAsyncKeyState(self.key_rapid_fire) < 0:
            return True
        return False
